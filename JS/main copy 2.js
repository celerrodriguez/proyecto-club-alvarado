...
const carrito = [];
const total = 0
//const cantidad = novedades.stock - productoAgregado.cantidad
...
function agregarAlCarrito(name) {
    ...
    for(let i =0; i < carrito.length; i+=1){
        if(carrito[i].nombre, trim() === name.nombre.trim()){
            carrito[i].cantidad +=1;
            const inputValue = productoAgregado[i]
            inputValue.valie+=1;
            return null;
        }
    }
...
contenedorCarrito.innerHTML = "";
    ...
    mCarrito.forEach(mostrarC => {
        ...
        const cantidadNovedades = document.createElement('h4');
        cantidadNovedades.classList.add('cantidad-novedades');
        cantidadNovedades.textContent = mostrarC.cantidad;
